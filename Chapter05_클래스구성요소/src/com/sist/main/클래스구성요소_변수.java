 ...
 *       => 구현이 불가능
 *          ---
 *           제어문 / 연산처리 / 파일읽기 / 크롤링 / 메소드 호출 ... => 메소드 처리 , 생성자 , 초기화블록을 이용
 *       => 선언만 가능하다
 *          -----------
 *          1. 변수 : 한개만 저장이 가능
 *          --------------------------
 ...
 *         }
 *         ** 접근지정어
 *         private : 자신의 클래스안에서만 사용 => 다른 클래스와 연결할 수 없다 => 멤버변수 (데이터 보호)
 *                                         --------------------
 *         default : 키워드가 없는 상태 => 같은 패키지안에서만 접근이 가능 
 *         protected : 같은 패키지안에서만 접근이 가능 + 상속을 받은 경우에는 다른 패키지 접근이 가능
 *         public : Open모드 => 패키지와 관계없이 모든 클래스에서 접근이 가능 
 *        
 ...
 *           ------- ---- -----      ----------- 0이상 => 매개변수 3개이상 추가하지 않는다 
 *                        | 기본형                               -----
 *                        | 배열                                 | 배열 / 클래스
 *                        | 클래스 
 *            public static | abstract
 *                   ------- 공통으로 사용 (한글 변환, 암호화, 복호화)
 *          {
 *                구현
 *          }
 *          
 ...
 *                                 String toUpperCase() => 대문자로 변환후에 문자열
 *         ---------------------------
 ...
 *                           ---
 *                           int / long / double => 권장 (같은 데이터형으로 받는다)
 ...
 *              Stack             Heap
 *             ---aa---
 *               0x100 => 메모리주소에 있는 메소드나 변수를 가지고 온다 => . => aa. (.은 메모리 주소 접근 연산자)
 *             --------     0x100 -------------
 *             
 *                                 -----------
 *                                      0  ---> a  ==> 객체명.변수명 => aa.a
...
public class 클래스구성요소_변수 {
		 ...
		 *           => aaa() / a / b
		 *       }
		 *       인스턴스메소드는 클래스에 있는 모든 데이터/메소드 사용이 가능
		 *    }
		 *    
