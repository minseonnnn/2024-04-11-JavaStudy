 ...
 *                           -------- 생략할 수 없는 내용 
 *                                    --------------
 *                                     상속을 내리는 클래스의 생성자가 매개변수를 가지고 있는 경우에는 생략할 수 없다
 *                        }
 *                     }     
 ...
 *                     -----
 *                       | 에러방지 => 에러는(소스상에서 수정할 수 없는 에러 / 예외처리(소스상에서 수정이 가능한 에러) 
 ...
 *       }                                      
 *                        
 *       => 주로 사용되는 곳은 클래스마다 기능은 동일한데 => 구현내용이 틀린 경우 => 선언 => 유지보수가 편리하게 만든다
 *       => 사이트마다 약간씩 틀리다 ...                  
 *          -----------------
 *           로그인 , 로그아웃 ... , 회원가입 ... 
 *           => 네이버 => Ajax
 ...
 *        => 오버라이딩 프로그램
 *           ------- 일반 클래스, 추상 클래스, 인터페이스  
 *           = 메소드명 동일 , 매개변수 동일 , 리턴형 동일 , 접근지정어는 다를 수 있다 (확장)
 *           
 *        abstract class A
 *        {
 ...
 *           
 *           (public abstract) void display(); => 선언만된 메소드만 가지고 있다 
 *        }
 *        
...
// D : 하위클래스 => this, super , C : 상위클래스 => this, super
// 상속을 받아서 새로운 확장된 클래스 만들어서 사용 
class D extends C
{
	public D()
	{
		super(10); // 상위 클래스가 매개변수가 있는 경우에는 반드시 super를 이용해서 호출해야 사용이 가능
		// 사용자 정의는 거의 없다 => 라이브러리에서 호출시에 반드시 super() 호출
		// super(),this() => 생성자 내에서 호출이 가능, 반드시 첫줄에서 호출한다 
		// => 둘중에 한개만 호출이 가능
		// 상위클래스 제어 => super , 생성자 super()
