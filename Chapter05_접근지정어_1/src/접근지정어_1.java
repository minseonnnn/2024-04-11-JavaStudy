 ...
 *      = 변수를 활용 : 메소드 => 기능 (수정,추가) => 다형성 (오버로딩 / 오버라이딩) => 웹 (메뉴) 
 ...
 *         데이터 선언
 *          => 멤버변수
 *             = 인스턴스변수 : new 사용시마다 메모리가 따로 생성 => 여러개 저장이 가능
 ...
 *                ==> 사용자 : 회원가입 (관리자)
 *                -------------------------- 유스케이스 다이어그램 
 *        Back-End (자바,JSP,Spring,오라클) => 데이터 관리 => 구현
 *        
 *        Front-End (JavaScript) => Ajax , Next , Vue , React => 객체지향 프로그램 => 화면 출력
 *        
 *        -----------------
 *         데이터 초기화 => 클래스 영역안에는 선언만 가능 => 구현(파일읽기,화면 UI,웹 연결,연산처리,제어문)
 ...
 *             = 여러개를 사용이 가능 => 오버로딩
 *             = 객체 생성시 반드시 호출 
 *             = 시작과 동시에 처리해야 하는 기능이 있는 경우 => 화면 UI
