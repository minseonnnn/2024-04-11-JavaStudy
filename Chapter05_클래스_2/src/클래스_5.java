 ...
 *            ** 공통변수가 사용되는 변수는 static를 이용한다
 *     생성자 : 클래스명과 동일 / 리턴형이 없다
 *            => 역할 메모리 할당할때 호출되는 메소드
 *            => 멤버변수의 초기화
 *               ------------ 서버 구동 / 화면 UI / 데이터베이스 연동
 *            => 반드시 필요하다 => 경우에 따라서 생략가능 => 컴파일러가 자동으로 생성자를 추가해 준다 
 ...
 *         ---- 기능 변경 / 추가 
 *              오버라이딩  오버로딩
 *     *** 객체 지향 프로그램 => OOP / => SM(유지보수) / SI(통합개발) 
 *         1) 캡슐화 : 데이터 보호 목적 => 파밍 => 크롤링 (X) => 해킹 => Vue/React/Ajax = 크롤링 불가
 ...
 *                                                         -------- 자바기반 
 *                                                         
 *     *** 프로그램이 비정상 종료가 안되게 만든다 : 예외처리 (8장) => 자바 기본 문법
 *     
 *                
 */
class Movie
