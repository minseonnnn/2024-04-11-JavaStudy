 ...
 *              입출력
 *               ---------------------------- 바이트 스트림 -------------------
 *               => 입력 메소드 : read() => 한글자 읽기 read() , read(byte[])
 ...
 *                                                          --------------
 *                 FileReader / FileWriter 
 *                 => 크롤링 후 파일에 저장 / 카페 , 블로그 .... 로그파일 => 분석 (추천)                                          
 *               ----------------------------------------------------------
 *                                                         
 *               ---------------------------- 필터 스트림 ---------------------
 ...
 *        => Properties
 *           ----------- 파일의 확장자 : .properties
 *                       구분자 있기 때문에 사용하기 편리하다 => 사용자 정의 
 *                       외부 라이브러리에서 주로 사용 : 보안이 뛰어난 것이 장점
 *                       -----------
 *                        | 데이터베이스 관련 (MyBatis, JPA , Spring) 
 *                                                       | => XML,Annotation
 *                          *** 필수적으로 사용 : 스프링 부트 (환경 설정)              
 *                                            | properties파일 / yml파일을 주로 이용
 *           작성법
 *           변수=값
 *           --- key역할을 수행 => 중복을 허용하지 않는다 
 *           --- 값은 중복 허용이 된다 
 *           id=admin
 *           name=홍길동 
 *           --------------- Map방식 
