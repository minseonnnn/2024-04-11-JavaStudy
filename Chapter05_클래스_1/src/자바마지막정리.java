 ...
 *          = 멤버변수 : new를 사용시마다 따로 저장되는 변수 (new)
 *                    = 자바에서 가장 많이 사용되는 변수  
 *          = 공유변수 : 저장공간이 1개라서 모든 클래스변수에서 공유해서 사용
 *                     static (컴파일시에 저장) => 오라클                                                
 *          ------------ 모든 사이트는 동일              
 *      *** 식별자
 ...
 *            | 배열 / 클래스 => 사용자 정의
 *         = 지역변수 : 기본형
 *         = 멤버변수 : String(문자열) + 기본형 => 배열(특별한 경우가 아니면 빈도수가 없다)
 *                                          -----------------------------
 *                                           고정 => 보완 => 컬렉션
 *      ---------------------------------- 데이터 저장
 *      데이터 처리
